...
const int pinIntrareADXL = 14; // Conectat la INT1 de pe ADXL345 (pentru alarme viitoare)
...
void IRAM_ATTR impulseRead()
{
  ...
  currentPeriod = currentTime - lastPeriod;
...
void loop() {
  ...
  // 1. Citim datele de la GPS fără să blocăm complet procesorul într-o buclă infinită
  while (serialGPS.available() > 0) {
    gps.encode(serialGPS.read());
  }

  // 2. Citim accelerometrul
  sensors_event_t event;
  accel.getEvent(&event);

  // 3. Verificăm dacă motorul s-a oprit (timeout de 250 ms de la aultimul impuls)
  if (micros() - lastPeriod > 250000) {
    currentPeriod = 0;
    rpmbun = 0; // Motorul este oprit cu siguranță
  }

  // 4. Calculăm RPM doar dacă avem o perioadă validă detectată de întrerupere
  if (currentPeriod > 0) {
    float freq =(float)currentPeriod / 1000000.0;

    float freq2 = 1.0/freq;
    int rpmCalculatAcum = (freq2 / 2.0) * 60;
    
    // Filtru simplu pentru zgomot
    if (rpmCalculatAcum > 100 && rpmCalculatAcum < 7500) {
      rpmbun = rpmCalculatAcum;
      sleepTimeout = timeC; // Resetăm somnul - motorul merge garantat!
    }
  }
